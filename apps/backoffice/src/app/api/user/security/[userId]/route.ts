...
export async function GET(
  ...
  try {
    ...
    if (cause instanceof TRPCError) {
      ...
      return Response.json({ message: "Internal server error", cause, stack: cause.stack }, { status: httpCode });
    }
    // Another error occurred
    console.error(cause);
    return Response.json({ message: "Internal server error", cause }, { status: 500 });
  }
};
